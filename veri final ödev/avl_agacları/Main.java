...
class BinaryTree { 
    ...
    { 
        ...
        lh = height(node.left); //ağacın leftinin yüksekliğini verir
        rh = height(node.right); //ağacın right alt ağacının yüksekliğini verir
  
        if (Math.abs(lh - rh) <= 1//abs mutlaktır
